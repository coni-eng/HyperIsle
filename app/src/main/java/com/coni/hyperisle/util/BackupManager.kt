...
import com.coni.hyperisle.data.model.AppSettingBackup
...
class BackupManager(
    ...
    suspend fun performExport(uri: Uri, selection: BackupSelection): Result<Boolean> = withContext(Dispatchers.IO) {
        try {
            ...
            // 4. Write
            val backup = HyperIsleBackup(metadata, filteredSettings)
    ...
    suspend fun readBackupFile(uri: Uri): Result<HyperIsleBackup> = withContext(Dispatchers.IO) {
        try {
            ...
            val backup = gson.fromJson(stringBuilder.toString(), HyperIsleBackup::class.java)
    ...
    suspend fun restoreBackup(backup: HyperIsleBackup, selection: BackupSelection): Result<Boolean> = withContext(Dispatchers.IO) {
        try {
